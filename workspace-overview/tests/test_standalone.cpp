...
// ===== Workspace Change Event Tests =====

struct WorkspaceChangeTestData {
    std::vector<int64_t> workspaceIDs;
    int64_t oldActiveID;
    int64_t newActiveID;
    size_t leftWorkspaceCount;
    size_t activeIndex;
};

// Helper to simulate workspace change and verify isActive flags
static void simulateWorkspaceChange(WorkspaceChangeTestData& data) {
    // Simulate images array with isActive flags
    std::vector<bool> isActive(data.leftWorkspaceCount + 1, false);

    // Set initial active flags
    for (size_t i = 0; i < data.leftWorkspaceCount; ++i) {
        if (i < data.workspaceIDs.size()) {
            isActive[i] = (data.workspaceIDs[i] == data.oldActiveID);
        }
    }
    isActive[data.activeIndex] = true;  // Right side is always active

    // Simulate the updateActiveWorkspaceDisplay logic
    for (size_t i = 0; i < data.leftWorkspaceCount; ++i) {
        if (i < data.workspaceIDs.size()) {
            isActive[i] = (data.workspaceIDs[i] == data.newActiveID);
        }
    }

    // Verify only the new active workspace is marked as active on left side
    for (size_t i = 0; i < data.leftWorkspaceCount; ++i) {
        if (i < data.workspaceIDs.size()) {
            bool expected = (data.workspaceIDs[i] == data.newActiveID);
            EXPECT_EQ(isActive[i], expected)
                << "Workspace at index " << i << " with ID " << data.workspaceIDs[i]
                << " should be " << (expected ? "active" : "inactive");
        }
    }
}

TEST(WorkspaceChangeEventTest, UpdateActiveWorkspaceFromFirstToSecond) {
    WorkspaceChangeTestData data;
    data.workspaceIDs = {1, 2, 3, 4};
    data.oldActiveID = 1;
    data.newActiveID = 2;
    data.leftWorkspaceCount = 4;
    data.activeIndex = 4;

    simulateWorkspaceChange(data);
}

TEST(WorkspaceChangeEventTest, UpdateActiveWorkspaceFromMiddleToLast) {
    WorkspaceChangeTestData data;
    data.workspaceIDs = {1, 2, 3, 4, 5, 6};
    data.oldActiveID = 3;
    data.newActiveID = 6;
    data.leftWorkspaceCount = 6;
    data.activeIndex = 6;

    simulateWorkspaceChange(data);
}

TEST(WorkspaceChangeEventTest, UpdateActiveWorkspaceToNonExistent) {
    // When switching to a workspace not in the left panel
    WorkspaceChangeTestData data;
    data.workspaceIDs = {1, 2, 3, 4};
    data.oldActiveID = 2;
    data.newActiveID = 7;  // Not in the list
    data.leftWorkspaceCount = 4;
    data.activeIndex = 4;
    ...
    // No workspace on left side should be active
    for (size_t i = 0; i < data.leftWorkspaceCount; ++i) {
        EXPECT_FALSE(isActive[i])
            << "Workspace at index " << i << " should be inactive when active workspace is not in list";
    }
}

TEST(WorkspaceChangeEventTest, NoChangeWhenSameWorkspace) {
    // Verify no update needed when workspace ID doesn't change
    int64_t oldActiveID = 3;
    int64_t newActiveID = 3;

    EXPECT_EQ(oldActiveID, newActiveID);
    // In the actual code, this triggers early return
}

TEST(WorkspaceChangeEventTest, UpdateActiveWorkspaceWithPlaceholders) {
    // Test with mix of real workspaces and placeholders
    WorkspaceChangeTestData data;
    data.workspaceIDs = {1, 2, 3, -1, -1};  // 3 real, 2 placeholders
    data.oldActiveID = 1;
    data.newActiveID = 3;
    data.leftWorkspaceCount = 5;
    data.activeIndex = 5;

    // Simulate images array with isActive flags
    std::vector<bool> isActive(data.leftWorkspaceCount + 1, false);

    // Set initial active flags (only for real workspaces)
    for (size_t i = 0; i < data.leftWorkspaceCount; ++i) {
        if (data.workspaceIDs[i] > 0) {
            isActive[i] = (data.workspaceIDs[i] == data.oldActiveID);
        }
    }

    // Simulate the update (only update real workspaces)
    for (size_t i = 0; i < data.leftWorkspaceCount; ++i) {
        if (data.workspaceIDs[i] > 0) {
            isActive[i] = (data.workspaceIDs[i] == data.newActiveID);
        }
    ...
    // Verify
    EXPECT_FALSE(isActive[0]);  // Workspace 1 (old active)
    EXPECT_FALSE(isActive[1]);  // Workspace 2
    EXPECT_TRUE(isActive[2]);   // Workspace 3 (new active)
    EXPECT_FALSE(isActive[3]);  // Placeholder
    EXPECT_FALSE(isActive[4]);  // Placeholder
}

TEST(WorkspaceChangeEventTest, RerenderOnlyAffectedWorkspaces) {
    // Verify that only workspaces whose active state changed need redrawing
    std::vector<int64_t> workspaceIDs = {1, 2, 3, 4, 5};
    int64_t oldActiveID = 2;
    int64_t newActiveID = 4;

    std::vector<bool> needsRedraw(workspaceIDs.size(), false);

    // Simulate the logic: redraw if active state changed and workspace is real
    for (size_t i = 0; i < workspaceIDs.size(); ++i) {
        bool wasActive = (workspaceIDs[i] == oldActiveID);
        bool isActive = (workspaceIDs[i] == newActiveID);
        if (wasActive != isActive && workspaceIDs[i] > 0) {
            needsRedraw[i] = true;
        }
    }

    // Only workspace 2 (index 1) and workspace 4 (index 3) should need redraw
    EXPECT_FALSE(needsRedraw[0]);  // Workspace 1 - no change
    EXPECT_TRUE(needsRedraw[1]);   // Workspace 2 - was active, now inactive
    EXPECT_FALSE(needsRedraw[2]);  // Workspace 3 - no change
    EXPECT_TRUE(needsRedraw[3]);   // Workspace 4 - was inactive, now active
    EXPECT_FALSE(needsRedraw[4]);  // Workspace 5 - no change
}
